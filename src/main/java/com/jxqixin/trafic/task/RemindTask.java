...
public class RemindTask {
    ...
    public void remindDay(){
        try {
            ...
            findWarningDeviceArchive(remindList,now);
            ...
            findExpiredDeviceArchive(remindList,now);
            if (!CollectionUtils.isEmpty(remindList)) {
                remindService.saveAll(remindList);
                remindList.clear();
            }
     ...
     * @param now
     */
    private void findWarningDeviceArchive(List<Remind> remindList,Date now){
        List<DeviceArchive> deviceArchiveList = remindService.findWarningDeviceArchives();
        if(!CollectionUtils.isEmpty(deviceArchiveList)){
            deviceArchiveList.forEach(deviceArchive -> {
                Remind remind = new Remind();
                remind.setBgColor("#FF9900");
                remind.setCreateDate(now);
                ...
                remind.setSrcId(deviceArchive.getId());
                remind.setSubjectName("设备档案");
     ...
     * @param now
     */
    private void findExpiredDeviceArchive(List<Remind> remindList,Date now){
        List<DeviceArchive> deviceArchiveList = remindService.findExpiredDeviceArchives();
        if(!CollectionUtils.isEmpty(deviceArchiveList)){
            deviceArchiveList.forEach(deviceArchive -> {
                Remind remind = new Remind();
